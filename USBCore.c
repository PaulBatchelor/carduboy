...
static void WaitOUT(void)
{
	while (!(UEINTX & (1<<RXOUTI)))
		;
}

static u8 WaitForINOrOUT()
{
...
static void Send8(u8 d)
{
	UEDATX = d;
}

static void SetEP(u8 ep)
{
	UENUM = ep;
}

static u8 FifoByteCount()
{
	return UEBCLX;
}

static u8 ReceivedSetupInt()
{
	return UEINTX & (1<<RXSTPI);
}

static void ClearSetupInt()
{
...
bool ClassInterfaceRequest(USBSetup *setup)
{
	...
	return pluggable_usb_setup(setup);
#endif
	return false;
}
...
static u8 SendInterfaces()
{
    ...
    pluggable_usb_get_interface(&interfaces);
#endif
...
bool SendDescriptor(USBSetup *setup)
{
...
#ifdef PLUGGABLE_USB_ENABLED
	ret = pluggable_usb_get_descriptor(setup);
	if (ret != 0) {
	...
	else if (USB_STRING_DESCRIPTOR_TYPE == t)
	{
		if (setup->wValueL == 0) {
            ...
            pluggable_usb_get_shortname(name);
